...
export async function updateDestBucketPolicy(bucketName, principal, roleArn, s3) {
  ...
  try {
    ...
    logger.warning(`Failed to get policy of bucket "${bucketName}. No policy is assumed to be present.`);
  }
...
/**
 * Extract the bucket name from the given bucket ARN.
 * @param {string} bucketArn Bucket ARN.
 * @returns Bucket name, or empty string otherwise.
 */
export function extractBucketName(bucketArn) {
  return bucketArn.match(/arn:aws:s3:::(.*)/)?.[1] ?? '';
}